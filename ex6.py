    ...
    def log_likelihood(self, X: np.ndarray) -> np.ndarray:
        ...
        if self._learn_cov:
            log_likelihood = np.array(
                [multivariate_normal.logpdf(X, mean=self._mu_ks[v_k], cov=self._sig_ks[v_k]) for v_k in range(self._k)])
        else:
            common_value = self._d * np.log(2 * np.pi * self._beta)
            x_k = np.array([X - mu_k for mu_k in self._mu_ks])
            inner = np.array([np.linalg.norm(x, axis=1) ** 2 for x in x_k])
            log_likelihood = -.5 * (
                        common_value + self._inv_beta * inner)  # L[j,i] : log likelihood of cluster j of point i
