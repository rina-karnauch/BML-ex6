...
def outlier_regression(model: BayesianLinearRegression, X: np.ndarray, y: np.ndarray, p_out: float, T: int,
    ...
    p_k_arr = np.zeros(X.shape)
    for i_T in range(T):
        for i, x in enumerate(X):
            y_i = y[i]
            p_k = p_outlier_per_x_T(p_outliers, mu_o, sig_o, model, y_i)
            p_k_arr[i] = p_k
            model.fit(X, y, True)
    outlier_indices = np.argwhere(p_k_arr > 0.5)
    return model, outlier_indices


def p_outlier_per_x_T(p_outlier, mu_o, sig_o, model, y_i):
    # model after fit to theta_T new
    normal_o = norm.pdf(y_i, mu_o, sig_o)
    normal_r = norm.pdf(y_i, model.fit_mu, model.fit_cov)
    nominator = p_outlier * normal_o
    denominator = nominator + (1 - p_outlier * normal_r)
